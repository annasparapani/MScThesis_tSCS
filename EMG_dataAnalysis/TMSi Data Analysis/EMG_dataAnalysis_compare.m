...
for rec=1:num_recordings
    ...
    filteredDataAll{rec}=filteredData;
end 
...
noArtDataAll=cell(num_recordings,1); 
for rec=1:(num_recordings) 
    noArtData=filteredDataAll{rec};
    filteredData=filteredDataAll{rec}(1:end-2,:); 
    noArtData=noArtData(1:end-2,:); %remove trigger and bipolar quad lines
    if data_struct{rec}.stim==1
        ...
        for i=1:size(noArtData,1)
            subplot(3,4,i), plot(filteredData(i,:)), ylabel('µV'),xlabel('time(s)'), title(titles_muscles(i));
            hold on; 
...
%% Offset removal
rectOffDataAll=cell(num_recordings,1); 

for rec=1:num_recordings
    noOffData=noArtDataAll{rec};
    noOffData=noOffData - mean(noOffData,2);
    rectOffDataAll{rec}=noOffData;
end

%% Rectification

rectDataAll=cell(num_recordings,1); 
for rec=1:num_recordings
    noArtData=rectOffDataAll{rec};
    rectData=abs(noArtData);
    if plots_on
        figure('Name', titles_recs{rec})
        for i=1:size(rectData,1)
            subplot(3,3,i), plot(rectData(i,:)), ylabel('µV'),xlabel('time(s)'), title(titles_muscles(i)+" rectified");
        end 
    end
    rectDataAll{rec}=rectData;
end

%% Envelope
envelopeDataAll=cell(num_recordings,1); 
for rec=1:num_recordings
    rectData=rectDataAll{rec};
    envelopeData=[];filterFreq = 50; filterOrder = 5; % Low-pass filtering 100 Hz, 5 order
    [B, A] = butter(filterOrder, filterFreq/(fs/2), 'low'); 
    for i=1:size(rectData,1)
        envelopeData(i,:) = filtfilt(B, A, rectData(i,:)); 
    end 
    figure('Name', titles_recs{rec}); %print envelope of QUAD DX 
    plot(rectData(1,:)), ylabel('µV'),xlabel('time(s)'), title(titles_muscles(1)+" envelope");
    hold on
    ...
    if plots_on
        figure('Name', titles_recs{rec});
        for i=1:size(rectData,1)
            subplot(3,3,i), plot(rectData(i,:)), ylabel('µV'),xlabel('time(s)'), title(titles_muscles(i)+" envelope");
            hold on; 
    ...
    envelopeDataAll{rec} = envelopeData; 
end 
%% COMPARE ACTIVATIONS 
...
for rec = 1:num_recordings
    ...
    smoothedData=[]; 
    envelopeData = envelopeDataAll{rec};
    for i = 1:size(envelopeData,1)
        smoothedData(i,:) = movmean(envelopeData(i,:), 20000);
    end
    smoothedDataAll{rec} = smoothedData;  
end 

for i=1:8
    figure('Name', titles_muscles{i})
    for rec = 1:num_recordings
        xAxis = time(1:length(smoothedDataAll{rec}(i,triggerLocsAll{rec}(2):end)));
        if rec==7
            plot(xAxis, smoothedDataAll{rec}(i,triggerLocsAll{rec}(2):end),'r','LineWidth',2)
        else plot(xAxis, smoothedDataAll{rec}(i,triggerLocsAll{rec}(2):end),'LineWidth',0.8)
        end 
...
for rec=1:num_recordings
    meanActivations{rec} = mean(envelopeDataAll{rec},2);
end

column_names={'Muscle', '50Hz','50 Hz + vol', '80Hz', '80Hz+vol','20Hz',...
    '20Hz+vol', 'Passive Cyling'};
T = table(titles_muscles(1:8)', meanActivations{1}, meanActivations{2}, ...
...
segmentedDataAll=cell(num_recordings,1);

for rec=1:num_recordings
    cycle_locs = triggerLocsAll{rec};
    segmentedDataAll{rec}.window_size = round(mean(diff(cycle_locs)));
    segmentedDataAll{rec}.average_cadence = 60/(window_size/fs);
    ...
    segmentedDataAll{rec}.Data=segmentedData;
end 
...
for rec = 1:num_recordings
    timecycle=linspace(0, size(segmentedDataAll{rec}.Data{muscle,segment},2)/fs, size(segmentedDataAll{rec}.Data{muscle,segment},2));
    title(titles_muscles(muscle))
    if rec == 7 
       plot(timecycle, movmean(segmentedDataAll{rec}.Data{muscle,segment},2000), 'LineWidth',2), xlabel('time(s)'), ylabel('EMG (µV)'); 
    else plot(timecycle, movmean(segmentedDataAll{rec}.Data{muscle,segment},2000), 'LineWidth',1), xlabel('time(s)'), ylabel('EMG (µV)'); 
    end 
    hold on; 
end 
legend(titles_recs)

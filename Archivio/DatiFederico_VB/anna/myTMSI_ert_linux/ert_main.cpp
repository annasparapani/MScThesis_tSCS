...
#include <signal.h>
#include "myTMSI.h"                    // Model's header file
...
void abortHandler(int sig)
{
  ...
  rtmSetStopRequested(myTMSI_M, 1);
}
...
#endif
{
  ...
  // External mode
  rtSetTFinalForExtMode(&rtmGetTFinal(myTMSI_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(myTMSI_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(myTMSI_M, true);
    }
  }
  ...
  while (rtmGetErrorStatus(myTMSI_M) == NULL && !rtmGetStopRequested(myTMSI_M))
  {
...
int_T main(int_T argc, const char_T *argv[])
{
  ...
  // Terminate model
  myTMSI_terminate();
  errStatus = rtmGetErrorStatus(myTMSI_M);
...
// Local Variables:
// compile-command: "make -f myTMSI.mk"
// End:

...
[filename, pathname, filterindex] = uigetfile('C:\Users\utente\Documents\Politecnico\TESI\MatLab_EMG\VB\.mat', 'Pick a MATLAB  file');

load(filename)

%%
EMG=[];
x=EMGdata.signals.values;

%%
for j = 1:16

    % EMGdata is a 3D matrix with 16 channels (14 muscles: 2 monopolar = 16) 
...
subplot(4,1,3), plot(time,x(:,3)), ylabel('Gast DX'), grid on %17
subplot(4,1,4), plot(time,x(:,4)), ylabel('TA DX'), grid on %18

figure()
title('LEFT LEG')
subplot(4,1,1), plot(time,x(:,5)), ylabel('Quad SX'), grid on %17
subplot(4,1,2), plot(time,x(:,6)), ylabel('Ham SX'), grid on %18
subplot(4,1,3), plot(time,x(:,7)), ylabel('Gast SX'), grid on %17
subplot(4,1,4), plot(time,x(:,8)), ylabel('TA SX'), grid on %18
...
% figure()
% %subplot(2,1,1), 
% plot(time,x_filt(:,1)), ylabel('Quad SX'), grid on; %17
% %subplot(2,1,2),
% figure()
% plot(time,x_filt(:,2)), ylabel('Ham SX'), grid on; %18
% 
% 
% %subplot(2,1,1), 
% figure()
% plot(time,x_filt(:,3)), ylabel('Quad DX'), grid on; %19
% %subplot(2,1,2),
% figure()
% plot(time,x_filt(:,4)), ylabel('Ham Dx'), grid on; %20 
...
%%
figure()
plot(time, xfilter(:,1)), ylabel('mV'), xlabel('s'), title('Quad Dx'), grid off;

figure()
plot(time, xfilter(:,2)), ylabel('mV'), xlabel('s'), title('Hams Dx'), grid off;

figure()
plot(time, xfilter(:,3)), ylabel('mV'), xlabel('s'), title('Gast Dx'), grid off;

figure()
plot(time, xfilter(:,4)), ylabel('mV'), xlabel('s'), title('TA Dx'), grid off;

figure()
plot(time, xfilter(:,5)), ylabel('mV'), xlabel('s'), title('Quad Sx'), grid off;

figure()
plot(time, xfilter(:,6)), ylabel('mV'), xlabel('s'), title('Hams Sx'), grid off;

figure()
plot(time, xfilter(:,7)), ylabel('mV'), xlabel('s'), title('Gast Sx'), grid off;

figure()
plot(time, xfilter(:,8)), ylabel('mV'), xlabel('s'), title('TA Sx'), grid off;
...
%%
  figure()
 plot(quad_dx), hold on, plot(locs1,pk1,'o'), ylabel('mV'), xlabel('samples');

 figure()
 plot(hams_dx), hold on, plot(locs2,pk2,'o'), ylabel('mV'), xlabel('samples');

   figure()
 plot(gast_dx), hold on, plot(locs3,pk3,'o'), ylabel('mV'), xlabel('samples');
